...
public class PlayerController : MonoBehaviour
{
    ...
    private Coroutine emoteDisplayCoroutine;
    ...
    public bool isFreezedByLevelController = false;
    public bool isAbovePlatform = false;
    ...
    public GameObject speechBallonObj;
    public Sprite[] allEmotes;
    public GameObject emoteDisplayObj;
    public SpriteRenderer emoteSpriteRenderer;
    public ParticleSystem reachParticles;
    [Space(8)]
    ...
    private void CheckIfIsOnGround()
    {
        ...
        //Calculate the distance of ground raycast checker. If is on a platform, the distance is greather to avoid false out-ground detection because gravity of platform
        float rayDistance = 0.0f;
        if (isAbovePlatform == false)
            rayDistance = 0.5f;
        if (isAbovePlatform == true)
            rayDistance = 2.0f;

        //Launch a raycast to check if have a ground below the player
        bool hasHit = Physics.Raycast(groundCheckRaycast.position, groundCheckRaycast.TransformDirection(Vector3.down), out raycastHitResult, rayDistance, groundCheckLayer);
    ...
    private void Movement(float currentInput)
    {
        ...
        //Calculate the additional artifical gravity, if is on platform to minimize the player gap between platform and player, because of gravity
        float platformArtificialGravity = 0.0f;
        if (isOnGround == true && isAbovePlatform == true)
            platformArtificialGravity = -400.0f;
        if (isOnGround == false || isAbovePlatform == false)
            platformArtificialGravity = 0.0f;

        //If is running... AND is not in a dash impulse AND not attacking AND is not falling into void AND is not death AND is not in a damage throw AND is not climbing AND is not freezed by level controller...
        if (currentInput != 0 && isOnDashImpulse == false && isAttacking == false && isFallingToVoid == false && isDead == false && isOnDamageThrow == DamageThrowTo.None && isClimbingGroundEdge == false && isFreezedByLevelController == false)
        {
            //Play the animation
            if (isOnWater == false)
            {
            ...
            if (isOnJump1Impulse == false && isOnJump2Impulse == false)
                playerRigidbody.velocity = transform.TransformVector(new Vector3(movementSpeed * currentInput, (artificialGravity + platformArtificialGravity), 0));
        ...
        //If is not running... OR is in dash impulse OR is attacking OR is falling into void OR is dead OR is in a damage throw OR is climbing OR is freezed by level controller...
        if (currentInput == 0 || isOnDashImpulse == true || isAttacking == true || isFallingToVoid == true || isDead == true || isOnDamageThrow != DamageThrowTo.None || isClimbingGroundEdge == true || isFreezedByLevelController == true)
        {
            ...
            if (isOnDashImpulse == false && isOnAttackImpulse == false && isOnDamageThrow == DamageThrowTo.None && isClimbingGroundEdge == false)
            {
                //Stop the movement
                if (isOnJump1Impulse == false && isOnJump2Impulse == false)
                    playerRigidbody.velocity = new Vector3(0, (artificialGravity + platformArtificialGravity), 0);
                if (isOnJump1Impulse == true || isOnJump2Impulse == true)
    ...
    private void Jumping(bool requestingJump)
    {
        ...
        if (isClimbingGroundEdge == true)
            return;
        //If is freezed by level controller, ignore
        if (isFreezedByLevelController == true)
            return;
    ...
    public void CauseDamage(float damageToCause, DamageType damageType)
    {
        ...
        if (currentHealth > damageToCause)
        {
            ...
            if (damageType == DamageType.WordFail)
                blowParticles.Play();
    ...
    public void ShowEmote(EmoteType emoteType)
    {
        //Try to decide the emote to use
        if (emoteType == EmoteType.Thinking)
            emoteSpriteRenderer.sprite = allEmotes[5];
        if (emoteType == EmoteType.Smile1)
            emoteSpriteRenderer.sprite = allEmotes[0];
        if (emoteType == EmoteType.Smile2)
            emoteSpriteRenderer.sprite = allEmotes[2];
        if (emoteType == EmoteType.Smile3)
            emoteSpriteRenderer.sprite = allEmotes[3];
        if (emoteType == EmoteType.Sad1)
            emoteSpriteRenderer.sprite = allEmotes[1];
        if (emoteType == EmoteType.Sad2)
            emoteSpriteRenderer.sprite = allEmotes[4];

        //If have a emote coroutine, reset it
        if (emoteDisplayCoroutine != null)
            StopCoroutine(emoteDisplayCoroutine);
        emoteDisplayCoroutine = null;
        emoteDisplayObj.SetActive(false);
        emoteDisplayCoroutine = StartCoroutine(ShowEmoteWaitAndHideAgain());
    }
    ...
    private IEnumerator DieBecauseOfAnythingThatIsNotVoid()
    {
        ...
        hudController.speakStepsObj.SetActive(false);
    ...
    private IEnumerator ClimbReleaseAvailabilityMonitor()
    {
        ...
        Vector2 startingPosition = new Vector2(thisTransform.position.x, thisTransform.position.y);

        //Create a loop to monitor the player height and make the climb ready after fall X meters down or move X meters left or right
        while (true)
        {
            //Get the current height and position
            //float currentHeight = thisTransform.position.y;
            //Get the current position
            Vector2 currentPosition = new Vector2(thisTransform.position.x, thisTransform.position.y);

            //Get the height difference between starting and current height
            //float heightDifference = startingHeight - currentHeight;

            //If is falled more than 3 meters, make the climbing ready again...
            //if (heightDifference >= 5.5f)
            //    break;
            //If is more far than 5.5 meters, make the climbing ready again...
            if (Vector2.Distance(startingPosition, currentPosition) >= 5.5f)
                break;
    ...
    private IEnumerator ShowEmoteWaitAndHideAgain()
    {
        //Show the emote
        emoteDisplayObj.SetActive(true);
