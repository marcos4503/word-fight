...
public class PlatformArea : MonoBehaviour
{
    ...
    public Rigidbody playerRigidbodyInHere;
    ...
    void Update()
    {
        ...
        if (Application.isPlaying == false)
        {
            ...
            if (platformStart.localPosition.x > -0.001f)
                platformStart.localPosition = new Vector3(-0.001f, platformStart.localPosition.y, platformStart.localPosition.z);
            if (platformEnd.localPosition.x < 0.001f)
                platformEnd.localPosition = new Vector3(0.001f, platformEnd.localPosition.y, platformEnd.localPosition.z);
        ...
        {
            //Make the player stays in the same position that he is entered on the platform, (if is not running AND not jumping)
            ...
            if (playerControllerInHere.isRunning == true)
                onEnterLocalPosition = playerTransformInHere.localPosition;

            //If the player is below the ground height of the platform OR is not in a ground OR is not in platform more, release the player from the platform
            if (playerTransformInHere.position.y < (mobilePlatformTransform.position.y + 0.7f) || playerControllerInHere.isOnGround == false || playerControllerInHere.isAbovePlatform == false)
            {
                //If is entering on platform, in a first jump, ignore
                if (playerControllerInHere.primeJumpReady == false || playerControllerInHere.doubleJumpReady == false)
                    return;

                //Inform that player is not above the platform
                playerControllerInHere.isAbovePlatform = false;
                //Inform that don't have a player
                playerTransformInHere.SetParent(null);
                playerTransformInHere = null;
                playerControllerInHere = null;
                playerRigidbodyInHere = null;
